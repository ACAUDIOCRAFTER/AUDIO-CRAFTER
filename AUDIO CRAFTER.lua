-- +======================================================+
-- |  AC | AudioCrafter  V3.6                            |
-- |  Made by MelodyCrafter                              |
-- +======================================================+
-- FIXES applied this revision:
--  FIX: Misc tab do..end block was never closed (broke Home page rendering)
--  FIX: Duplicate open animation block removed (caused stutter/FPS drop)
--  FIX: rS2 undefined in buildUgcRow -> renamed to rS (was always rS)
--  FIX: _uiOpen=true now set correctly in single open animation block
--  FIX: States slider had duplicate InputBegan connection on sKnb
--  FIX: ugcPanel height constant UGC_H now used consistently
...
-- In-game startup notification (Xeno/AKADMIN style)
pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "AudioCrafter";
        Text = "v3.6 Attached!  by MelodyCrafter";
...
do
    ...
    -- FIX: Use valid Roblox UI sound IDs (rbxasset:// are built-in, never 429 or CrossExperience error)
    ...
    AC._uiOpen=false  -- starts false; set true after open animation completes
end
...
do
    ...
    -- FIX: minBtn has visible dash icon, larger hit area, ZIndex=10 so navbar children don't block it
    AC.minBtn=Instance.new("TextButton",AC.navbar)
    AC.minBtn.Size=UDim2.new(0,28,0,28); AC.minBtn.Position=UDim2.new(1,-34,0.5,-14)
    AC.minBtn.BackgroundColor3=Color3.fromRGB(40,40,40); AC.minBtn.Text="-"
    AC.minBtn.TextColor3=AC.TXT_WHITE; AC.minBtn.TextSize=18; AC.minBtn.Font=Enum.Font.GothamBold
    AC.minBtn.ZIndex=10; AC.minBtn.AutoButtonColor=false
    Instance.new("UICorner",AC.minBtn).CornerRadius=UDim.new(0,6)
    -- Toggle UI button: no hover/click sounds, visual feedback only
    ...
    AC.doOpen=function()
        ...
        AC.reopenBtn.Visible=false; AC.wrapper.Visible=true; AC.wrapper.Size=UDim2.new(0,0,0,0)
    ...
    -- Dynamic toggle keybind - uses AC._toggleKey (default "G", changeable in Player tab)
    AC.UIS.InputBegan:Connect(function(i,gp)
        if gp then return end
        local ok,kn=pcall(function() return i.KeyCode.Name end)
        if ok and kn and kn==(AC._toggleKey or "G") then
            if AC._uiOpen then AC.doMin() else AC.doOpen() end
        end
    end)
end

-- FPS/PING HUD + TAGS  (stacked vertical card, fully draggable)
do
    local HUD_W=120; local ROW_H=26; local HUD_H=ROW_H*3+8

    local hud=Instance.new("Frame",AC.screenGui)
    hud.Size=UDim2.new(0,HUD_W,0,HUD_H)
    hud.Position=UDim2.new(0,10,0,10)   -- top-left, drag anywhere
    hud.BackgroundColor3=Color3.fromRGB(8,8,10)
    hud.BackgroundTransparency=0.12
    hud.ZIndex=300; hud.ClipsDescendants=true
    ...
    -- Left accent bar
    local acBar=Instance.new("Frame",hud); acBar.Size=UDim2.new(0,3,1,-8); acBar.Position=UDim2.new(0,4,0,4)
    acBar.BackgroundColor3=AC.PUR_BRIGHT; acBar.BorderSizePixel=0
    ...
    local function makeRow(yOff, icon, label, iconColor, textColor)
        local row=Instance.new("Frame",hud)
        row.Size=UDim2.new(1,-10,0,ROW_H); row.Position=UDim2.new(0,10,0,4+yOff)
        row.BackgroundTransparency=1; row.ZIndex=301
        -- divider line above rows 2 and 3
        if yOff>0 then
            local div=Instance.new("Frame",hud)
            div.Size=UDim2.new(1,-14,0,1); div.Position=UDim2.new(0,10,0,4+yOff)
            div.BackgroundColor3=AC.PUR_STROKE; div.BackgroundTransparency=0.5
        end
        local dot=Instance.new("Frame",row); dot.Size=UDim2.new(0,7,0,7); dot.Position=UDim2.new(0,0,0.5,-3)
        dot.BackgroundColor3=iconColor; dot.BorderSizePixel=0; Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
        local lbl=Instance.new("TextLabel",row)
        lbl.Size=UDim2.new(1,-12,1,0); lbl.Position=UDim2.new(0,12,0,0)
        lbl.BackgroundTransparency=1; lbl.Text=label
        lbl.TextColor3=textColor; lbl.TextSize=11; lbl.Font=Enum.Font.GothamBold
        lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=302
        return lbl
    end

    local tagsLbl=makeRow(0,        "Tags", "Tags",   AC.PUR_BRIGHT, AC.PUR_BRIGHT)
    local fpsLbl =makeRow(ROW_H,    "FPS",  "FPS 60", AC.GREEN_OK,   AC.GREEN_OK)
    local pingLbl=makeRow(ROW_H*2,  "PING", "PING --", AC.PUR_GLOW,  AC.PUR_GLOW)

    -- Tags row is a button (whole row area)
    local tagsBtn=Instance.new("TextButton",hud)
    tagsBtn.Size=UDim2.new(1,0,0,ROW_H); tagsBtn.Position=UDim2.new(0,0,0,4)
    tagsBtn.BackgroundTransparency=1; tagsBtn.Text=""; tagsBtn.ZIndex=303
    tagsBtn.MouseButton1Click:Connect(function()
        ...
        for _,p in ipairs(AC.Players:GetPlayers()) do
            if p.Character then local h=p.Character:FindFirstChild("Head"); if h then local bb2=h:FindFirstChild("AC_Billboard"); if bb2 then bb2.Enabled=AC.tagsVisible end end end
        end
        local onCol=AC.tagsVisible and AC.PUR_BRIGHT or AC.TXT_DIM
        tagsLbl.Text=AC.tagsVisible and "Tags  ON" or "Tags  OFF"
        tagsLbl.TextColor3=onCol
    ...
    -- Fully free drag - no clamping so it reaches every corner
    local hudDrag,hudDS,hudSP=false,nil,nil
    hud.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then
            hudDrag=true; hudDS=i.Position; hudSP=hud.Position
            i.Changed:Connect(function() if i.UserInputState==Enum.UserInputState.End then hudDrag=false end end)
        end
    end)
    AC.UIS.InputChanged:Connect(function(i)
        if hudDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
            local d=i.Position-hudDS
            local nx=math.clamp(hudSP.X.Offset+d.X, 0, AC.camera.ViewportSize.X-HUD_W)
            local ny=math.clamp(hudSP.Y.Offset+d.Y, 0, AC.camera.ViewportSize.Y-HUD_H)
            hud.Position=UDim2.new(0,nx,0,ny)
        end
    end)
...
do
    ...
    -- Owner UserId: only THIS player gets the gold "AC OWNER" tag
    local OWNER_ID = AC.player.UserId
    local TYPE_SPD=0.09; local HOLD=1.8; local DEL_SPD=0.04; local BLANK=0.35

    -- Typewriter that works with any display text passed in
    local function startTypewriter(tLbl, displayText)
        task.spawn(function()
            while tLbl and tLbl.Parent do
                tLbl.Text="|"
                for i=1,#displayText do
                    if not (tLbl and tLbl.Parent) then return end
                    tLbl.Text=displayText:sub(1,i).."|"
                    task.wait(TYPE_SPD)
                end
                if not (tLbl and tLbl.Parent) then return end
                tLbl.Text=displayText.."|"
                task.wait(HOLD)
                for i=#displayText,0,-1 do
                    if not (tLbl and tLbl.Parent) then return end
                    tLbl.Text=displayText:sub(1,i).."|"
                    task.wait(DEL_SPD)
                end
    ...
    local function attachTag(char, owner)
        ...
        -- Determine if this is the owner (the person running the script)
        local isOwner = (owner and owner.UserId == OWNER_ID)

        -- Owner gets gold, everyone else gets purple
        ...
        local pill=Instance.new("Frame",bb); pill.Size=UDim2.new(1,0,1,0)
        pill.BackgroundColor3=bgColor; pill.BackgroundTransparency=0.2
        ...
        -- Logo: gold for owner, purple for others
        local logoFrame=AC.drawLogo(pill,18,tagColor)
        logoFrame.Position=UDim2.new(0,6,0.5,-9)

        local titleLbl=Instance.new("TextLabel",pill)
        titleLbl.Size=UDim2.new(1,-28,0,18); titleLbl.Position=UDim2.new(0,26,0,2)
        ...
        titleLbl.TextStrokeColor3=isOwner and Color3.fromRGB(80,50,0) or AC.PUR_DARK

        local userLbl=Instance.new("TextLabel",pill)
        userLbl.Size=UDim2.new(1,-28,0,14); userLbl.Position=UDim2.new(0,26,0,20)
        userLbl.BackgroundTransparency=1
        userLbl.Text="@"..(owner and owner.Name or "AC User")
        userLbl.TextColor3=isOwner and Color3.fromRGB(255,200,80) or Color3.fromRGB(200,170,220)
        ...
        grad.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,gradCol0),ColorSequenceKeypoint.new(1,gradCol1)}
        grad.Rotation=135

        -- Pulsing glow on owner tag
        if isOwner then
            AC.TS:Create(pStroke,TweenInfo.new(1.1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.5}):Play()
        end
    ...
    local function watchPlayer(p)
        ...
        local function onChar(char)
            task.spawn(function()
                local head=char:WaitForChild("Head",10); if not head then return end
                -- FIX: always attach tag for ALL players immediately, no marker required
                -- The marker was only used for detecting OTHER AC users, not for showing tags
                attachTag(char,p)
            end)
        end
...
do
    ...
    local ws=Instance.new("TextLabel",wCard); ws.Size=UDim2.new(1,-20,0,20); ws.Position=UDim2.new(0,14,0,44); ws.BackgroundTransparency=1; ws.Text="AC AudioCrafter v3.6  by MelodyCrafter"; ws.TextColor3=AC.PUR_MID; ws.TextSize=12; ws.Font=Enum.Font.Gotham; ws.TextXAlignment=Enum.TextXAlignment.Left
    ...
    ic("Version","v3.6",0); ic("Status","* Active",1,AC.GREEN_OK); ic("Script",AC.executorName,2,AC.PUR_BRIGHT)
    ...
    cll("v3.6  Current Update",12,AC.PUR_GLOW,Enum.Font.GothamBold)
    cll("FIX: Misc tab do..end block never closed (was breaking Home page render).",10,AC.TXT_MAIN)
    cll("FIX: Duplicate open animation block removed (caused stutter/FPS drop on execute).",10,AC.TXT_MAIN)
    cll("FIX: rS2 undefined variable in buildUgcRow renamed to rS.",10,AC.TXT_MAIN)
    cll("FIX: _uiOpen now correctly set after open animation completes.",10,AC.TXT_MAIN)
    cll("FIX: States slider duplicate InputBegan connection on sKnb removed.",10,AC.TXT_MAIN)
    cll("FIX: Billboard LOD system removed (was crashing with _head invalid member).",10,AC.TXT_MAIN)
    cll("FIX: All syntax errors fixed (extra end, missing end, else do blocks).",10,AC.TXT_MAIN)
    cll("FIX: All slider RS connections now on-demand (connect/disconnect on drag).",10,AC.TXT_MAIN)
    cll("FIX: UGC favs tab now correctly shows only favorited emotes.",10,AC.TXT_MAIN)
    cll("FIX: Target dropdown now works correctly.",10,AC.TXT_MAIN)
    cll("v3.1  Previous",12,AC.PUR_MID,Enum.Font.GothamBold)
    ...
    cll("Stop emote fix: api.stop_animation() called correctly.",10,AC.TXT_MAIN)
    ...
    cll("Billboard: typewriter loop, lighter purple.",10,AC.TXT_MAIN)
...
do
    ...
    local function rebuildDropdown(query)
            ...
            if p~=AC.player then
                if q=="" or p.Name:lower():find(q,1,true) or p.DisplayName:lower():find(q,1,true) then
                    ...
                    row.MouseButton1Click:Connect(function()
                        ...
                        task.delay(0.3, function() ddSelecting=false end)
                    end)
        ...
        -- FIX: recompute AbsolutePosition every open (was always 0,0 at build time)
        local ddH=math.min(count*38+8,180)
        local ap=sc.AbsolutePosition; local as=sc.AbsoluteSize
        ddFrame.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+2)
        ddFrame.Size=UDim2.new(0,as.X,0,ddH)
    ...
    -- FIX: single task.wait instead of double task.defer - gives enough time for click to fire
    ...
    -- TOGGLE UI KEYBIND BUTTON: row 2 right column, same row as Clear Target
    local PAD2,GAP2=12,6; local bW2=(AC.MAIN_W-PAD2*2-GAP2)/2; local bH2=42
    local kbFrame=Instance.new("Frame",pg)
    kbFrame.Size=UDim2.new(0,bW2,0,bH2)
    kbFrame.Position=UDim2.new(0,PAD2+1*(bW2+GAP2),0,S+2*(bH2+6))
    kbFrame.BackgroundColor3=AC.BG_CARD
    ...
    -- invisible click button over the whole frame
    ...
    -- load saved keybind
    local _savedToggleKey=pcall(function()
        if isfile and isfile("BLEED/"..tostring(AC.player.UserId).."/toggle_key.json") then
            local d=AC.Http:JSONDecode(readfile("BLEED/"..tostring(AC.player.UserId).."/toggle_key.json"))
            if d and d.key then AC._toggleKey=d.key; kbValLbl.Text="["..d.key.."]" end
        end
    end)
    if not AC._toggleKey then AC._toggleKey="G" end
    kbBtn.MouseButton1Click:Connect(function()
        if _kbListening then
            -- cancel
            _kbListening=false; if _kbConn then _kbConn:Disconnect(); _kbConn=nil end
            kbValLbl.Text="["..AC._toggleKey.."]"; kbValLbl.TextColor3=AC.PUR_GLOW
            kbTopLbl.Text="Toggle UI Key"; kbStr.Color=AC.PUR_STROKE; return
        end
        -- start listening
        _kbListening=true
        kbValLbl.Text="..."; kbValLbl.TextColor3=AC.ORANGE_W
        kbTopLbl.Text="Press any key"
        kbStr.Color=AC.PUR_BRIGHT
        _kbConn=AC.UIS.InputBegan:Connect(function(inp,gp)
            if gp or inp.UserInputType~=Enum.UserInputType.Keyboard then return end
            local ok,kn=pcall(function() return inp.KeyCode.Name end)
            if not ok or not kn or kn=="Unknown" then return end
            if kn=="Escape" then
            ...
            pcall(function()
                ...
                local dir="BLEED/"..tostring(AC.player.UserId)
                ...
                writefile(dir.."/toggle_key.json", AC.Http:JSONEncode({key=kn}))
            end)
            AC.toast("Toggle key set to ["..kn.."]",AC.PUR_BRIGHT)
            task.delay(1.2,function() kbValLbl.TextColor3=AC.PUR_GLOW end)
        end)
    end)
    ...
    siBtn.MouseButton1Click:Connect(function()
        if AC.onHead then
            AC.onHead=false; if AC.headConn then AC.headConn:Disconnect(); AC.headConn=nil end
            local h2=AC.player.Character and AC.player.Character:FindFirstChildOfClass("Humanoid")
            if h2 then h2.Sit=false end; siBtn.Text="Sit on Head"; AC.toast("Off head",AC.ORANGE_W); return
        end
        ...
        AC.headConn=AC.RS.Heartbeat:Connect(function()
            ...
            local mc=AC.player.Character; local mr=mc and mc:FindFirstChild("HumanoidRootPart")
            local mh=mc and mc:FindFirstChildOfClass("Humanoid")
            local th=AC.selectedTarget.Character and AC.selectedTarget.Character:FindFirstChild("Head")
            if mr and th then if mh then mh.Sit=true end; mr.CFrame=th.CFrame*CFrame.new(0,th.Size.Y+0.8,0) end
        end)
    end)
    -- BACK-TO-BACK: player stands directly behind target, both facing same direction (like a backpack)
    baBtn.MouseButton1Click:Connect(function()
        if AC.inBp then
            ...
            if mh then mh.Sit=false end; baBtn.Text="Back-to-Back"; AC.toast("Back-to-Back OFF",AC.ORANGE_W); return
        end
        if not AC.selectedTarget or not AC.selectedTarget.Character then AC.toast("No target",AC.RED_ERR); return end
        ...
        AC.bpConn=AC.RS.Heartbeat:Connect(function()
            ...
            if mr and tr then
                if mh2 then mh2.Sit=true end
                -- FIX: back-to-back = behind the target, facing SAME direction as target (not facing them)
                -- offset 0,0,2 = directly behind them in target's local space + slightly elevated
                mr.CFrame=tr.CFrame*CFrame.new(0,0.5,2)
            end
        end)
    end)
    clBtn.MouseButton1Click:Connect(function()
        AC.stopViewing(); AC.onHead=false; AC.inBp=false; AC.selectedTarget=nil
        sBox.Text=""; siBtn.Text="Sit on Head"; baBtn.Text="Back-to-Back"; AC.toast("Target cleared")
    end)

    -- PLAYER INFO: below buttons, big green hacker text
    -- Buttons: S=98, 3 rows (removed brBtn+foBtn) of bH=42+6gap=48 -> bottom at 98+3*48=242, +8=250
    -- But we keep layout safe at 250
    local INFO_Y=258
    AC.sectionLbl(pg,"PLAYER INFO",INFO_Y)
    local infoCard=AC.makeCard(pg,INFO_Y+18,AC.BG_CARD); infoCard.Size=UDim2.new(1,-24,0,120)
    Instance.new("UIGradient",infoCard).Color=ColorSequence.new{
        ColorSequenceKeypoint.new(0,Color3.fromRGB(0,12,0)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(0,6,0))
    }
    -- Green scan-line accent bar on left
    local accentBar=Instance.new("Frame",infoCard); accentBar.Size=UDim2.new(0,3,1,-8); accentBar.Position=UDim2.new(0,6,0,4)
    accentBar.BackgroundColor3=Color3.fromRGB(0,255,80); accentBar.BorderSizePixel=0
    Instance.new("UICorner",accentBar).CornerRadius=UDim.new(1,0)
    local infoLbl=Instance.new("TextLabel",infoCard)
    infoLbl.Size=UDim2.new(1,-26,1,-8); infoLbl.Position=UDim2.new(0,18,0,4)
    infoLbl.BackgroundTransparency=1
    infoLbl.Text="> SELECT A TARGET"
    infoLbl.TextColor3=Color3.fromRGB(80,255,80)  -- bright hacker green
    infoLbl.TextSize=13; infoLbl.Font=Enum.Font.Code
    infoLbl.TextXAlignment=Enum.TextXAlignment.Left
    infoLbl.TextWrapped=true; infoLbl.RichText=true
    local function updatePlayerInfo(p)
        if not p then
            infoLbl.Text="> SELECT A TARGET"
            infoLbl.TextColor3=Color3.fromRGB(60,160,60)
            return
        end
        local uid=tostring(p.UserId)
        local acctAge=p.AccountAge
        local years=math.floor(acctAge/365)
        local months=math.floor((acctAge%365)/30)
        local days2=acctAge%30
        local ageStr=(years>0 and years.."y " or "")..(months>0 and months.."mo " or "")..days2.."d"
        -- Team color gives a hint of their team
        local teamStr="None"
        pcall(function() if p.Team then teamStr=p.Team.Name end end)
        -- Check if they have a character loaded
        local charStr=p.Character and "LOADED" or "NOT LOADED"
        infoLbl.Text="> NAME    : "..p.Name.."\n> DISPLAY : "..p.DisplayName.."\n> USER ID : "..uid.."\n> AGE     : "..ageStr.." ("..acctAge.." days)\n> CHAR    : "..charStr.."  TEAM: "..teamStr
        infoLbl.TextColor3=Color3.fromRGB(0,255,80)
    end
    local _lastTarget=nil
    AC.RS.Heartbeat:Connect(function()
        if AC.selectedTarget~=_lastTarget then
            _lastTarget=AC.selectedTarget
            updatePlayerInfo(AC.selectedTarget)
        end
    end)
...
do
    ...
    local bW2=math.floor((RN_W-16-4*4)/5)
    for i,nv in ipairs(NUM_VALS) do
        local xOff=(i-1)*(bW2+8)
        local nb=Instance.new("TextBox",numRowF)
        ...
        nb.Text=""; nb.PlaceholderText=tostring(nv); nb.PlaceholderColor3=Color3.fromRGB(80,80,100)
        nb.TextColor3=AC.TXT_WHITE; nb.TextSize=11; nb.Font=Enum.Font.GothamBold; nb.ZIndex=72
        ...
        nb.FocusLost:Connect(function(enter)
            if enter and nb.Text~="" then
                ...
                if v and v>0 then setRnSpd(math.clamp(v,0.1,12)) end
            end
        end)
    ...
    do
        local fl=readData("rn_favs.json",{})
        if type(fl)=="table" then for _,id in ipairs(fl) do rnFavs[tostring(id)]=true end end
        local bl=readData("rn_binds.json",{})
        if type(bl)=="table" then for k,v in pairs(bl) do rnBinds[k]=v end end
        local sl=readData("rn_speeds.json",{})
        if type(sl)=="table" then for k,v in pairs(sl) do rnSpeeds[k]=v end end
    end
    ...
    local function buildRnRow(name,url,idx)
        ...
        local rS=Instance.new("UIStroke",row); rS.Color=AC.PUR_STROKE; rS.Thickness=1; rS.Transparency=0.7  -- FIX: was rS, now consistently rS
        ...
        row.MouseButton1Click:Connect(function()
            ...
            ensureAPI(function(api)
                if highlighted then
                    ...
                    rS.Color=AC.PUR_STROKE; rnHighRow=nil  -- FIX: was rS2, now rS
                    AC.toast("Stopped: "..nameC,AC.ORANGE_W); return
                end
                if rnHighRow then
                    pcall(function()
                        AC.TS:Create(rnHighRow,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play()
                        local ps=rnHighRow:FindFirstChildOfClass("UIStroke"); if ps then ps.Color=AC.PUR_STROKE end
                    end)
                end
                pcall(function() api.play_animation(urlC,emoteSpd) end); rnNowUrl=urlC; highlighted=true; rnHighRow=row
                AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=AC.PUR_DARK}):Play(); rS.Color=AC.PUR_BRIGHT  -- FIX: was rS2
    ...
    ugcPanel.Size=UDim2.new(0,UGC_W,0,UGC_H); ugcPanel.Position=UDim2.new(0.5,-UGC_W/2,0.15,0)
    ...
    ugcBottomBar.Size=UDim2.new(1,-16,0,118); ugcBottomBar.Position=UDim2.new(0,8,1,-126)
    ...
    local ugcNumRowF=Instance.new("Frame",ugcBottomBar); ugcNumRowF.Size=UDim2.new(1,-16,0,56); ugcNumRowF.Position=UDim2.new(0,8,0,58); ugcNumRowF.BackgroundTransparency=1; ugcNumRowF.ZIndex=67
    ...
    for i,nv in ipairs(UGC_NUM_VALS) do
        ...
        local nb=Instance.new("TextBox",ugcNumRowF)
        nb.Size=UDim2.new(0,ugcNBW,0,22); nb.Position=UDim2.new(0,xOff,0,0)
        nb.BackgroundColor3=Color3.fromRGB(20,20,24); nb.BackgroundTransparency=0.2
        nb.Text=""; nb.PlaceholderText=tostring(nv); nb.PlaceholderColor3=Color3.fromRGB(80,80,100)
        nb.TextColor3=AC.TXT_WHITE; nb.TextSize=11; nb.Font=Enum.Font.GothamBold; nb.ZIndex=68
        nb.ClearTextOnFocus=false
        Instance.new("UICorner",nb).CornerRadius=UDim.new(0,5); Instance.new("UIStroke",nb).Color=AC.PUR_STROKE
        nb.FocusLost:Connect(function(enter)
            if enter and nb.Text~="" then
                local v=tonumber(nb.Text)
                if v and v>0 then setUgcSpd(math.clamp(v,0.1,10)) end
            end
        end)
    ...
    local ugcLoadedTracks = {}
    local function stopUgcTrack()
        if ugcActiveTrack then
            pcall(function() ugcActiveTrack:Stop(0); ugcActiveTrack:Destroy() end)
            ugcActiveTrack=nil
        end
        for _,t in ipairs(ugcLoadedTracks) do
            pcall(function() if t then t:Stop(0); t:Destroy() end end)
        end
        ugcLoadedTracks={}
        pcall(function()
    ...
    local function playUgcEmote(id)
        ...
        if ok and track and typeof(track)=="Instance" and track:IsA("AnimationTrack") then
            ugcActiveTrack=track; track.Looped=true
        ...
        if ok2 and t2 then
            for _,existing in ipairs(animator:GetPlayingAnimationTracks()) do
                pcall(function() existing:Stop(0); existing:Destroy() end)
            end
            t2.Priority=Enum.AnimationPriority.Action4
            t2.Looped=true; t2:Play(0)
            pcall(function() t2:AdjustSpeed(ugcSpdCur) end)
            ugcActiveTrack=t2
        end
    end
    ...
    local function buildUgcRow(item,idx)
        ...
        row.MouseButton1Click:Connect(function()
            if highlighted then
                stopUgcTrack()
                highlighted=false
                if ugcHighRow==row then ugcHighRow=nil end
                AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play()
                rS.Color=AC.PUR_STROKE  -- FIX: was rS2 (undefined), now correctly rS
                AC.toast("Stopped",AC.ORANGE_W); return
            end
            if ugcHighRow and ugcHighRow~=row then
                pcall(function()
                    AC.TS:Create(ugcHighRow,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(14,14,18)}):Play()
                    local oldStroke=ugcHighRow:FindFirstChildOfClass("UIStroke")
                    if oldStroke then oldStroke.Color=AC.PUR_STROKE end
                end)
                ugcHighRow=nil
            end
            stopUgcTrack()
            playUgcEmote(item.id)
            highlighted=true; ugcHighRow=row
            AC.TS:Create(row,TweenInfo.new(0.15),{BackgroundColor3=AC.PUR_DARK}):Play()
            rS.Color=AC.PUR_BRIGHT  -- FIX: was rS2 (undefined), now correctly rS
    ...
    for _,cat in ipairs(STATE_CATS) do
        ...
        -- FIX: single drag connection on sKnb (was duplicated in original)
        local _sDC=nil
        sKnb.InputBegan:Connect(function(i)
            if i.UserInputType~=Enum.UserInputType.MouseButton1 then return end
            stDrag=true
            _sDC=AC.RS.RenderStepped:Connect(function()
                if not stDrag then return end
                setStSpd(0.1+(AC.UIS:GetMouseLocation().X-sTrk.AbsolutePosition.X)/sTrk.AbsoluteSize.X*4.9)
            end)
        end)
        ...
        AC.UIS.InputEnded:Connect(function(i)
            if i.UserInputType~=Enum.UserInputType.MouseButton1 then return end
            stDrag=false; if _sDC then _sDC:Disconnect(); _sDC=nil end
        end)
    ...
    AC.UIS.InputChanged:Connect(function(i)
        if ugcDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            local newY=math.clamp(ugcSP.Y.Offset+d.Y, 0, AC.camera.ViewportSize.Y-UGC_H)
    ...
    ugcMainBtn.MouseButton1Click:Connect(function()
        ...
        if ugcPanel.Visible then
            ...
            if pos.X.Offset<0 or pos.X.Offset>vp.X-50 or pos.Y.Offset<0 or pos.Y.Offset>vp.Y-50 then
                ugcPanel.Position=UDim2.new(0.5,-UGC_W/2,0.5,-UGC_H/2)
            end
            loadUgcData()
        end
    end)
...
-- MISC TAB  -- FIX: this do..end block was never closed in the original, breaking all subsequent code
do
    ...
    local JOINT_PAIRS={{"Head","UpperTorso"},{"UpperTorso","LowerTorso"},{"UpperTorso","LeftUpperArm"},{"UpperTorso","RightUpperArm"},{"LowerTorso","LeftUpperLeg"},{"LowerTorso","RightUpperLeg"},{"Head","Torso"},{"Torso","Left Arm"},{"Torso","Right Arm"},{"Torso","Left Leg"},{"Torso","Right Leg"}}
    local ESP_RED=Color3.fromRGB(160,60,255)
    ...
    local function addESP(p)
        ...
        for _,pair in ipairs(JOINT_PAIRS) do
            ...
            if pA and pA:IsA("BasePart") and pB and pB:IsA("BasePart") then
                ...
                beam.Color=ColorSequence.new(ESP_RED)
                beam.Width0=0.07; beam.Width1=0.07
                beam.FaceCamera=true; beam.Segments=1
                beam.Transparency=NumberSequence.new(0)
                beam.LightEmission=1; beam.Parent=char
                table.insert(beams,{beam=beam,att0=a0,att1=a1})
            end
        end
        -- Name tag above head with actual name text
        local head=char:FindFirstChild("Head")
        if head then
            local nbb=Instance.new("BillboardGui")
            nbb.Name="AC_ESP_Name"
            nbb.Size=UDim2.new(0,160,0,36)
            nbb.StudsOffset=Vector3.new(0,2.4,0)
            nbb.AlwaysOnTop=true; nbb.ResetOnSpawn=false; nbb.MaxDistance=0
            nbb.Adornee=head; nbb.Parent=head  -- parent to head so it persists

            local bg=Instance.new("Frame",nbb)
            bg.Size=UDim2.new(1,0,1,0)
            bg.BackgroundColor3=Color3.fromRGB(80,0,160)
            bg.BackgroundTransparency=0.35; bg.BorderSizePixel=0
            Instance.new("UICorner",bg).CornerRadius=UDim.new(0,7)
            local stroke=Instance.new("UIStroke",bg)
            stroke.Color=AC.PUR_BRIGHT; stroke.Thickness=1.5; stroke.Transparency=0.1

            -- Player username (top)
            local nameLbl=Instance.new("TextLabel",bg)
            nameLbl.Size=UDim2.new(1,-8,0,18); nameLbl.Position=UDim2.new(0,4,0,1)
            nameLbl.BackgroundTransparency=1
            nameLbl.Text=p.Name
            nameLbl.TextColor3=Color3.new(1,1,1)
            nameLbl.TextSize=13; nameLbl.Font=Enum.Font.GothamBold
            nameLbl.TextXAlignment=Enum.TextXAlignment.Center
            nameLbl.TextStrokeTransparency=0.4; nameLbl.TextStrokeColor3=Color3.fromRGB(40,0,80)
            nameLbl.TextScaled=false; nameLbl.TextTruncate=Enum.TextTruncate.AtEnd

            -- Display name / distance (bottom)
            local dispLbl=Instance.new("TextLabel",bg)
            dispLbl.Size=UDim2.new(1,-8,0,14); dispLbl.Position=UDim2.new(0,4,0,19)
            dispLbl.BackgroundTransparency=1
            dispLbl.Text="@"..p.DisplayName
            dispLbl.TextColor3=AC.PUR_GLOW
    ...
    local espOn=false
    local _,_,onESP=AC.makeToggle(pg,"Highlight Usernames",28,false)
    ...
    local cr1=Instance.new("TextLabel",crd); cr1.Size=UDim2.new(1,-16,0,20); cr1.Position=UDim2.new(0,12,0,6); cr1.BackgroundTransparency=1; cr1.Text="AC AudioCrafter V3.6  by MelodyCrafter"; cr1.TextColor3=AC.PUR_BRIGHT; cr1.TextSize=13; cr1.Font=Enum.Font.GothamBold; cr1.TextXAlignment=Enum.TextXAlignment.Left
...
-- OPEN ANIMATION (single, fixed)
do
    -- FIX: Hide wrapper completely first so switchTab flash is invisible, THEN switch tab.
    ...
    -- Switch to home now while invisible - guaranteed to show home when UI opens
    AC.switchTab("Home")
    task.delay(0.1,function()
        AC.wrapper.Visible=true  -- FIX: make visible before tweening so Home page shows
        pcall(function() AC.openSound:Play() end)
    ...
    task.delay(1.5,function() AC.toast("AC AudioCrafter v3.6 loaded!  G = toggle") end)
    print("AC AudioCrafter V3.6  by MelodyCrafter")
